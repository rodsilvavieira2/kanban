...
export const useKanbanStore = create<KanbanState>((set, get) => ({
  ...
  loadProjectData: async (projectId: string) => {
    ...
    try {
      const { columns, tasks } = await window.kanbanApi.getProjectData(projectId);
      set({ columns, tasks, isLoading: false });
    } catch (error: any) {
      set({ error: error.message || 'Failed to load project data', isLoading: false });
    }
  },
  ...
  loadAllTasks: async () => {
    ...
    try {
      ...
      for (const project of projects) {
        const { tasks } = await window.kanbanApi.getProjectData(project.id);
  ...
  createTask: async (projectId: string, taskData: Partial<Task>) => {
    try {
      const newTask = await window.kanbanApi.createTask(taskData);
      set((state) => ({
  ...
  moveTask: async (request) => {
    ...
    if (sourceColumnId === destinationColumnId) {
      const columnTasks = newTasks.filter(t => t.columnId === sourceColumnId).sort((a, b) => a.order - b.order);
      ...
      columnTasks.forEach((t, index) => {
        ...
        if (originalIndex !== -1) {
          newTasks[originalIndex] = { ...newTasks[originalIndex], order: index };
        }
      });
    } else {
      const sourceTasks = newTasks.filter(t => t.columnId === sourceColumnId).sort((a, b) => a.order - b.order);
      const destTasks = newTasks.filter(t => t.columnId === destinationColumnId).sort((a, b) => a.order - b.order);
      ...
      destTasks.forEach((t, index) => {
        const originalIndex = newTasks.findIndex(nt => nt.id === t.id);
        if (originalIndex !== -1) {
          newTasks[originalIndex] = { ...newTasks[originalIndex], order: index, columnId: destinationColumnId };
        }
      });
    }
    ...
    try {
      await window.kanbanApi.moveTask(request);
    } catch (error: any) {
  ...
  updateTaskTime: async (taskId: string, minutes: number) => {
    try {
      await window.kanbanApi.updateTaskTime(taskId, minutes);
      set((state) => ({
