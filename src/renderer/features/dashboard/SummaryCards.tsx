...
export function SummaryCards() {
  ...
  const inProgressColumnIds = new Set(
    columns
      .filter(c => /progress|doing/i.test(c.title))
      .map(c => c.id)
  );
  const inProgressTasks = tasks.filter(t => inProgressColumnIds.has(t.columnId)).length;
  ...
  const completedTasks = tasks.filter(t => completedColumnIds.has(t.columnId)).length;
  ...
  const CARDS = [
    ...
    },
    {
      ...
      trend: totalTasks > 0 ? `${Math.round((inProgressTasks / totalTasks) * 100)}% of total` : 'No tasks yet',
      trendType: 'neutral' as const,
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    },
    {
      ...
      trend: totalTasks > 0 ? `${Math.round((completedTasks / totalTasks) * 100)}% completion rate` : 'No tasks yet',
